    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"403 détecté. Pause de {delay:.2f} secondes.")
                time.sleep(delay)
            else:
                print(f"Erreur HTTP {response.status_code}. Arrêt.")
                break

        except requests.exceptions.RequestException as e:
            delay = min(base_delay * (2 ** attempt), max_delay) + random.uniform(0, 3)
            print(f"Erreur réseau : {e}. Pause de {delay:.2f} secondes.")
            time.sleep(delay)
        ...
        while retries < max_retries:
            try:
                ...
                if response.status_code == 200:  # Succès
                    print(f"Page {page_count} récupérée avec succès.")
                    ...
                    print(f"Code 403 détecté. Pause de {delay:.2f} secondes avant nouvelle tentative.")
                    time.sleep(delay)
                    retries += 1
                ...
                delay = min(base_delay * (2 ** retries), max_delay) + random.uniform(0, 3)
                print(f"Erreur réseau : {e}. Pause de {delay:.2f} secondes avant nouvelle tentative.")
                time.sleep(delay)
                retries += 1

        # Si les tentatives échouent après max_retries
        if retries >= max_retries:
            print(f"Échec du scraping de la page {page_count} après {max_retries} tentatives.")
            break
...
def scrape_restaurant(url):
    ...
    try:
        ...
        if not soup:
            print(f"Impossible de récupérer les données pour {url}")
            return None
        ...
        if not (name and address and reviews_count and rating and ranking):
            print("Données incomplètes, nouvelle tentative après pause.")
...
def scrape_restaurant_list(base_url, max_restaurants=15, min_reviews=900):
    ...
    while current_url and len(restaurant_urls) < max_restaurants:
        print(f"Scraping restaurant list, page {page_count + 1}...")
        response = requests.get(current_url, headers=HEADERS, timeout=10)
        if response.status_code != 200:
            print(f"Erreur HTTP {response.status_code} sur {current_url}. Arrêt du scraping.")
            break
        ...
        for card in restaurant_cards:
            ...
            if reviews_count >= min_reviews:
                ...
                if link and link.get('href'):
                    ...
                    print(f"Restaurant trouvé : {restaurant_url} avec {reviews_count} avis.")
    ...
    print(f"Scraping terminé. {len(restaurant_urls)} restaurants trouvés.")
